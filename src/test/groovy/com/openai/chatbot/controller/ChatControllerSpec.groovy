...
class ChatControllerSpec extends Specification {
    ...
    def "chat should return ResponseEntity with ChatResponse when service returns successfully"() {
        ...
        chatService.chat(request.message) >> serviceResponse

        when: "chat endpoint is called"
        def response = chatController.chat(request)

        then: "service is called once with correct message"
        1 * chatService.chat(request.message) >> serviceResponse
    ...
    def "chat should delegate message processing to ChatService"() {
        ...
        chatService.chat(userMessage) >> expectedResponse

        when: "chat endpoint is called"
        def response = chatController.chat(request)

        then: "service receives the exact user message"
        1 * chatService.chat(userMessage) >> expectedResponse
    ...
    def "chat should propagate exceptions from ChatService"() {
        ...
        chatService.chat(_) >> { throw exception }
    ...
    def "chat should handle empty message from request"() {
        ...
        chatService.chat("") >> serviceResponse

        when: "chat endpoint is called"
        def response = chatController.chat(request)

        then: "service is called with empty message"
        1 * chatService.chat("") >> serviceResponse
    ...
    def "chat should handle long messages"() {
        ...
        chatService.chat(longMessage) >> serviceResponse

        when: "chat endpoint is called"
        def response = chatController.chat(request)

        then: "service is called with the long message"
        1 * chatService.chat(longMessage) >> serviceResponse
    ...
    def "chat should handle special characters in message"() {
        ...
        chatService.chat(specialMessage) >> serviceResponse

        when: "chat endpoint is called"
        def response = chatController.chat(request)

        then: "service is called with the special message"
        1 * chatService.chat(specialMessage) >> serviceResponse
    ...
    def "chat should return response with correct model information"() {
        ...
        chatService.chat(_) >> serviceResponse
